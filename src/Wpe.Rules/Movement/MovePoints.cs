...
/// <summary>
/// movement / movePoints — units spend movement points to step into adjacent hexes;
...
public sealed class MovePoints : RuleVariantBase, IMovementModule
{
    ...
    public override void Load(string? configJson, GameDefinition def)
    {
        ...
        if (doc.RootElement.TryGetProperty("riverCrossCost", out var rcc))
        {
            if (rcc.ValueKind == JsonValueKind.Number) _riverCrossCost = rcc.GetSingle();
            else if (rcc.ValueKind == JsonValueKind.String && float.TryParse(rcc.GetString(), out var f)) _riverCrossCost = f;
        }
    }
    ...
    public float MoveCost(GameState state, HexCoord a, HexCoord b)
        => StepCost(state, a, b);
    ...
    public List<HexCoord> Reachable(CounterState unit, GameEngine engine)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            foreach (var n in HexMath.Neighbors(cur, gm.PointyTop))
            {
                ...
                var cost = curCost + StepCost(engine.State, cur, n);
